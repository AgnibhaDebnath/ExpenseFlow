...
const FilterDropdown = ({ label, items, isOpen, setToolbarDropdown, setFormDropdown, setTransactionCategory, setTransactionType, setDashboardFilterDropdown }) => {
                ...
                <motion.div
                    ...
                    >
                        {items.map((item) => {
                            ...
                            if (((label === "category-form" && item.value === "all categories") || (label === "type-form" && item.value === "all types"))) {
                                return null
                            }
                            return (
                                ...
                                <motion.button
                                    ...
                                    onClick={() => {
                                        if (label === "category") {
                                            setTransactionFilters(prev => ({ ...prev, category: item.value }))
                                            setToolbarDropdown(null)
                                        }
                                        else if (label === "type") {
                                            setTransactionFilters(prev => ({ ...prev, type: item.value }))
                                            setToolbarDropdown(null)
                                        } else if (label === "transaction-date-filter") {
                                            setTransactionFilters(prev => ({ ...prev, dateFilter: item.value }))
                                            setToolbarDropdown(null)
                                        }
                                        else if (label === "category-form") {
                                            setTransactionCategory(item.value)
                    ...
                    {(label === "category-form" || label == "Category") && <div className="pointer-events-none absolute bottom-0 left-0 h-10 w-full bg-linear-to-t from-white to-transparent" />}
                </motion.div>
